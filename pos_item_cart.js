erpnext.PointOfSale.ItemCart = class {
			...
			`<div class="cart-container">
				<div class="abs-cart-container">
					...
					<div class="cart-header">
						...
						<div class="qty-header">MRP</div>
						<div class="qty-header">Rate</div>
	...
	make_served_by_selector() {
		...
		this.customer_field = frappe.ui.form.make_control({
			df: {
				...
				onchange: function() {
					if (this.value) {
						const frm = me.events.get_frm();
						frappe.dom.freeze();
						frappe.model.set_value(frm.doc.doctype, frm.doc.name, 'name', this.value);
						frm.script_manager.trigger('customer', frm.doc.doctype, frm.doc.name).then(() => {
							frappe.run_serially([
								() => me.fetch_customer_details(this.value),
								() => me.events.customer_details_updated(me.customer_info),
			...
			parent: this.$customer_section.find('.served-by-field'),
			render_input: true,
		});
		this.customer_field.toggle_label(false);
	}
	...
	render_cart_item(item_data, $item_to_update) {
		...
		function get_rate_discount_html() {
			...
			return `
				<div class="item-qty-rate">
					<div class="item-qty"><span>${item_data.mrp || 0}</span></div>
