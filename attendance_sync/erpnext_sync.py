...
class Config():
    ...
    ERPNEXT_URL = attendance_config.api_url

    # operational configs
    PULL_FREQUENCY = 0 # in minutes
    LOGS_DIRECTORY = BASE_DIR + '/logs' # logs of this script is stored in this directory
    IMPORT_START_DATE = attendance_config.import_start_date # format: '20190501'
    ...
    devices = []
    for device in attendance_config.biometric_devices:
        devices.append({
            'device_id': device.device_id,
            'ip': device.device_ip,
            'punch_direction': device.punch_direction,
            'clear_from_device_on_fetch': True if device.clear_from_device_on_fetch == 1 else False
        })

    # Configs updating sync timestamp in the Shift Type DocType
    shift_type_device_mapping = []
    for shift in attendance_config.shift_types:
        related_devices = shift.related_devices
        shift_type_device_mapping.append({
            'shift_type_name': [shift.shift_type_name],
            'related_device_id': related_devices.split(',') if ',' in related_devices else [related_devices]
        })
...
def pull_process_and_push_data(device, device_attendance_logs=None):
    ...
    for device_attendance_log in device_attendance_logs[index_of_last+1:]:
        ...
        print(f"{erpnext_status_code} =======>> {erpnext_message}")
