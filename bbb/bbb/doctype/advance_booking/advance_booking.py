...
class AdvanceBooking(SalesInvoice):
    def __init__(self, *args, **kwargs):
        super(AdvanceBooking, self).__init__(*args, **kwargs)

    def validate(self):
        if not cint(self.is_pos):
            frappe.throw(
                _("Advance Booking should have {} field checked.").format(
        ...
        self.caculate_advance_booking_total()
        if self.coupon_code:
    ...
    def on_submit(self):
        ...
        if not self.is_return and self.loyalty_program:
            ...
            against_psi_doc = frappe.get_doc(
                "Advance Booking", self.return_against)
        ...
        for pay in self.payments:
            if pay.type == "Phone" and pay.amount >= 0:
                paid_amt = frappe.db.get_value(
                    "Payment Request",
                    filters=dict(
                        reference_doctype="Advance Booking",
    ...
    def validate_pos_reserved_serial_nos(self, item):
        ...
        if len(invalid_serial_nos) == 1:
            frappe.throw(
                _(
                    "Row #{}: Serial No. {} has already been transacted into another Advance Booking. Please select valid serial no."
            ...
            frappe.throw(
                _(
                    "Row #{}: Serial Nos. {} have already been transacted into another Advance Booking. Please select valid serial no."
                ).format(item.idx, bold_invalid_serial_nos),
                title=_("Item Unavailable"),
            )
    ...
    def validate_pos_reserved_batch_qty(self, item):
        filters = {"item_code": item.item_code,
                   "warehouse": item.warehouse, "batch_no": item.batch_no}

        available_batch_qty = get_batch_qty(
            item.batch_no, item.warehouse, item.item_code)
    ...
    def get_delivered_serial_nos(serial_no):
        delivered_serial_nos = frappe.db.get_list(
            "Delivery Note Item",
            filters={
                "serial_no": ["like", f"%{serial_no}%"]  # Check if serial_no is linked
            },
            pluck="serial_no"
        )
    ...
    def validate_delivered_serial_nos(self, item):
        ...
        if delivered_serial_nos:
            bold_delivered_serial_nos = frappe.bold(
                ", ".join(delivered_serial_nos))
            frappe.throw(
                _(
        ...
        for serial_no in serial_nos:
            if not frappe.db.exists("Serial No", serial_no):
                invalid_serials = invalid_serials + \
                    (", " if invalid_serials else "") + serial_no
        msg = _("Row #{}: Following Serial numbers for item {} are <b>Invalid</b>: {}").format(
            item.idx, frappe.bold(item.get("item_code")
                                  ), frappe.bold(invalid_serials)
        )
        if invalid_serials:
    ...
    def validate_stock_availablility(self):
        ...
        for d in self.get("items"):
            is_service_item = not (frappe.db.get_value(
    ...
    def validate_mode_of_payment(self):
        if len(self.payments) == 0:
            frappe.throw(
                _("At least one mode of payment is required for Advance Booking."))
    ...
    def validate_change_amount(self):
        ...
        if not flt(self.change_amount) and grand_total < flt(self.paid_amount):
            ...
            self.base_change_amount = (
                flt(self.base_paid_amount) - base_grand_total +
    ...
    def validate_loyalty_transaction(self):
        if self.redeem_loyalty_points and (
            ...
            expense_account, cost_center = frappe.db.get_value(
                "Loyalty Program", self.loyalty_program, [
    ...
    def set_status(self, update=False, status=None, update_modified=True):
        ...
        if not status:
            if self.docstatus == 2:
                ...
                if flt(self.outstanding_amount) > 0 and (getdate(self.actual_service_date) < getdate(nowdate())) and flt(self.total_advance) == 0 and flt(self.rounded_total) > 0:
                    self.billing_status = "Overdue"
                elif flt(self.outstanding_amount) == 0 and (getdate(self.actual_service_date) < getdate(nowdate())) and flt(self.total_advance) == 0 and flt(self.rounded_total) == 0:
                    self.billing_status = "Overdue and Discounted"
                    
                if flt(self.outstanding_amount) > 0 and (flt(self.outstanding_amount) == flt(self.rounded_total)):
                    self.status = "To Deliver and Bill"
                    self.billing_status = "Not Billed"

                elif flt(self.outstanding_amount) > 0 and (flt(self.outstanding_amount) < flt(self.rounded_total)) and flt(self.total_advance) > 0:
                    self.status = "To Deliver and Bill"
                    self.billing_status = "Partly Billed"

                elif flt(self.outstanding_amount) == 0 and (flt(self.total_advance) == flt(self.rounded_total)):
                    self.status = "To Deliver"
                    self.billing_status = "Fully Billed"

                elif flt(self.outstanding_amount) == 0 and flt(self.total_advance) == 0 and flt(self.rounded_total) == 0:
                    self.status = "To Deliver"
                    self.billing_status = "Not Billed and Discounted"
    ...
    def set_pos_fields(self, for_validate=False):
        ...
        if profile:
            ...
            if self.customer:
                ...
                selling_price_list = (
                    customer_price_list or customer_group_price_list or profile.get(
                        "selling_price_list")
                )
            ...
            for item in self.get("items"):
                if item.get("item_code"):
                    profile_details = get_pos_profile_item_details(
                        profile.get("company"), frappe._dict(
                            item.as_dict()), profile
                    )
    ...
    def set_missing_values(self, for_validate=False):
        ...
        if not print_format and not cint(frappe.db.get_value("Print Format", "Advance Booking", "disabled")):
            print_format = "Advance Booking"
    ...
    def caculate_advance_booking_total(self):
        total_advance = 0
...
def get_pos_reserved_qty(item_code, warehouse):
    reserved_qty = frappe.db.sql(
		...
		from `tabAdvance Booking` p, `tabAdvance Booking Item` p_item
...
def make_sales_return(source_name, target_doc=None):
    ...
    return make_return_doc("Advance Booking", source_name, target_doc)
...
def make_merge_log(invoices):
    ...
    merge_log = frappe.new_doc("Advance Booking Merge Log")
    merge_log.posting_date = getdate(nowdate())
    for inv in invoices:
        inv_data = frappe.db.get_values(
            "Advance Booking", inv.get("name"), ["customer", "posting_date", "grand_total"], as_dict=1
        )[0]
