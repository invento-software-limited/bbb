...
def execute(filters=None):
    ...
    if not filters.get('warehouse') or not filters.get('warehouse'):
        return [], []
...
def get_item_warehouse_map(filters, sle):
    ...
    for d in sle:
        ...
        elif d.posting_date >= from_date and d.posting_date <= from_date:
...
def get_item_details(items, sle, filters):
    ...
    res = frappe.db.sql(
        """
        select
            item.name, item.item_name, item.buying_rate, item.description, item.item_group, item.brand, item.stock_uom %s
        from
            `tabItem` item
            %s
        where
