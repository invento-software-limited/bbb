...
def execute(filters=None):
	columns, data = get_columns(), get_item_data()
	return columns, data
...
def get_columns():
    ...
    columns = [
        ...
        {"label": _("Category Name"), "fieldname": "category", "fieldtype": "Link", "width": 130, "options": "Item Group"},
        {"label": _("Sub Category"), "fieldname": "sub_category", "fieldtype": "Link", "width": 130, "options": "Item Group"},
...
def get_item_data():
    query = """select row_number() over (order by item.name) as serial_no, item.item_code as product_code, item.item_name as product_name, item.item_group as sub_category, item.brand,
     ...
     `tabItem Barcode` item_barcode on item_barcode.parent=item.name"""
