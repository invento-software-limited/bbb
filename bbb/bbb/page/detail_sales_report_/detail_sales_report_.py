...
def get_invoice_data(filters):
    ...
    for index, result in enumerate(query_result):
        total_profit+=(result.get('net_amount') - result.get('buying_total'))
        if data.get(result.get('name'), None):
            ...
            sales_data['profit_loss'] += (result.get('net_amount') - result.get('buying_total'))
            ...
            result['profit_loss'] = (result.get('net_amount') - result.get('buying_total'))
            data[result.get('name')] = result

    return data, query_result, total_profit


@frappe.whitelist()
def generate_table_data(filters):
    data, query_result, total_profit = get_invoice_data(filters)
    sales_data = {}
    total_item_qty = 0
    ...
    for index, result in enumerate(query_result):
        ...
        total_amount = result['rounded_total'] if invoice_item_total['total_return_amount'] == '' else \
            invoice_item_total['total_return_amount'] + result['rounded_total']
        ...
        tr_first_row = f"""
            <tr>
                ...
                <td rowspan='{invoice_item_total['total_item_row']}'>{invoice_item_total['profit_loss']}</td>
    ...
    sales_data['total_amount_row'] = [f"""
                        <tr class='font-weight-bold'>
                            ...
                            <td rowspan='2' colspan='2'>Total Profit={total_profit}</td>
