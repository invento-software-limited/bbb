...
@frappe.whitelist()
@frappe.validate_and_sanitize_search_inputs
def customer_query2(doctype, txt, searchfield, start, page_len, filters, as_dict=False):
	doctype = "Customer"
	conditions = []
    ...
    if args.get('warehouse'):
        ...
        return frappe.db.sql("""select name, concat('MFG-', manufacturing_date), concat('EXP-',expiry_date)
			...
			limit %(start)s, %(page_len)s""".format(cond, search_columns=search_columns,
                                                    search_cond=search_cond, match_conditions=get_match_cond(doctype)),
                             args)
...
def served_by_query(doctype, txt, searchfield, start, page_len, filters):
    ...
    return frappe.db.sql("""select {fields} from `tabServed By`
		...
		order by
			if(locate(%(_txt)s, name), locate(%(_txt)s, name), 99999),
			if(locate(%(_txt)s, served_by_name), locate(%(_txt)s, served_by_name), 99999),
			idx desc,
			name, served_by_name
		limit %(start)s, %(page_len)s""".format(**{
        "fields": ", ".join(fields),
