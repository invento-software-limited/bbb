...
class POSInvoiceMergeLog(Document):
	...
	def on_submit(self):
		...
		if returns:
			for return_data in returns:
				credit_note = self.process_merging_into_credit_note([return_data])
				self.save()  # save consolidated_sales_invoice & consolidated_credit_note ref in merge log
				self.update_pos_invoices(pos_invoice_docs, sales_invoice, credit_note)

		if sales:
			for sales_data in sales:
				sales_invoice = self.process_merging_into_sales_invoice([sales_data])
				self.save()  # save consolidated_sales_invoice & consolidated_credit_note ref in merge log
				self.update_pos_invoices(pos_invoice_docs, sales_invoice, credit_note)
	...
	def merge_pos_invoice_into(self, invoice, data):
		...
		invoice.set("outstanding_amount", doc.outstanding_amount)
...
def get_invoice_customer_map(pos_invoices):
	...
	for invoice in pos_invoices:
		...
		customer = invoice.get("customer") + 'POSINV#@' + invoice.name
...
def create_merge_logs(invoice_by_customer, closing_entry=None):
	try:
		...
		for customer_invoice, invoices in invoice_by_customer.items():
			# rabiul added this line
			customer = customer_invoice.split('POSINV#@')[0]
