...
def get_pricing_rule_for_item(args, price_list_rate=0, doc=None, for_validate=False):
	...
	if args.ignore_pricing_rule or not args.item_code:
		...
		return item_details
	# distribution
	update_distribution_discount(args, item_details)
	...
	if args.transaction_type == "selling":
		if args.customer and not (args.customer_group and args.territory):
			...
			if args.quotation_to and args.quotation_to != "Customer":
				...
				customer = frappe.get_cached_value("Customer", args.customer, ["customer_group", "territory", "distribution_discount_percentage"])

			if customer:
				# distribution
				# args.customer_group, args.territory = customer
				args.customer_group, args.territory, args.distribution_discount_percentage = customer
...
# distribution
def update_distribution_discount(args, item_details):
	if args.customer:
		customer = frappe.get_cached_value("Customer", args.customer, ["customer_group", "distribution_discount_percentage"])
		customer_group, distribution_discount_percentage = customer
		if customer and customer_group == 'Distribution' and distribution_discount_percentage > 0:
			item_details.customer_group, item_details.distribution_discount_percentage = customer
