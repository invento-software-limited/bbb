...
def get_item_details(args, doc=None, for_validate=False, overwrite_warehouse=True):
	...
	# distribution
	validate_and_update_pricing_rules_for_distribution(data)


	if data.get('free_item_data'):
		free_item_data = data.get('free_item_data')
		out.update({'free_item_rules': free_item_data[0]['pricing_rules'], 'min_qty': free_item_data[0]['min_qty'], 'free_qty': free_item_data[0]['free_qty']})
	else:
		out.update(data)
	...
	pricing_rule_for = out.get('pricing_rule_for', None)
	discount_amount = out.get('discount_amount', None)
	if discount_amount == 0 and pricing_rule_for == "Discount Amount":
		try:
			item_discount_amount, price_rule_tag = get_discount_amount(item_code, out.pricing_rules)
			if item_discount_amount:
				out['price_rule_tag'] = price_rule_tag
				out['discount_amount'] = item_discount_amount
		except:
			pass
	return out

# rabiul
# get discount amount base on item wise discount
def get_discount_amount(item_code, pricing_rules):
	item_doc = frappe.get_doc("Item", {"item_code": item_code})
	start_date = item_doc.start_date
	end_date = item_doc.end_date
	discount_amount = item_doc.discount_amount
	today = datetime.datetime.today().date()
	if pricing_rules:
		apply_on = None
		pricing_rules = json.loads(pricing_rules)
		for pricing_rule in pricing_rules:
			apply_on = frappe.db.get_value('Pricing Rule', pricing_rule, 'apply_on')
			# print('apply_on ', apply_on, pricing_rule)
		if start_date and end_date and start_date <= today and end_date >= today and apply_on == 'Tag':
			return discount_amount, item_doc.price_rule_tag
	...
	out = frappe._dict(
		{
			...
			"discount_amount": 0.0,
			"price_rule_tag": item.price_rule_tag,
...
def get_gross_profit(out):
	...
	discount_amount = out.get('discount_amount', None)
	if discount_amount == 0 and pricing_rule_for == "Discount Amount":
		item_discount_amount = get_discount_amount(item_code)
		if item_discount_amount:
			out['discount_amount'] = item_discount_amount
	return out
...
# rabiul
# get discount amount base on item wise discount
def get_discount_amount(item_code):
	item_doc = frappe.get_doc("Item", {"item_code": item_code})
	start_date = item_doc.start_date
	end_date = item_doc.end_date
	discount_amount = item_doc.discount_amount
...
# distribution
def validate_and_update_pricing_rules_for_distribution(data):
	pricing_rules = data.get('pricing_rules', None)
	if pricing_rules:
		pricing_rules = json.loads(pricing_rules)
		for pricing_rule in pricing_rules:
			try:
				customer_group = frappe.db.get_value('Pricing Rule', pricing_rule, 'customer_group')
				if customer_group == 'Distribution' and data.get('pricing_rule_for') == 'Discount Percentage':
					data['discount_percentage'] = data.get('distribution_discount_percentage')
			except:
				pass