(() => {
  ...
  // ../bbb/bbb/public/js/point_of_sale/pos_controller.bundle.js
  erpnext.PointOfSale.Controller = class {
    constructor(wrapper) {
      ...
      this.base_rounded_total = void 0;
      this.initial_paid_amount = void 0;
    }
    fetch_opening_entry() {
    ...
    check_item_stock() {
      function get_items_template(dialog, data) {
        ...
        var table_data = frappe.render_template(html, { data });
...
`);
    ...
    update_special_note() {
      ...
      if (special_note !== void 0 || special_note) {
    ...
    get_naming_series(me) {
      if (me.frm.doc.__islocal) {
        frappe.call({
          ...
          callback: function(r) {
            let random_number = Math.floor(Math.random() * 1e4 + 1);
      ...
      this.order_summary = new erpnext.PointOfSale.PastOrderSummary({
        ...
        events: {
          ...
          process_return: (name) => {
            ...
            frappe.db.get_doc("POS Invoice", name).then((doc) => {
              this.make_return_invoice(doc).then(function() {
                setTimeout(function() {
    ...
    async on_cart_update(args) {
      ...
      try {
        ...
        if (item_row_exists && free_item === void 0) {
          if (field === "qty")
            value = flt(value);
...
`).length || 0;
    ...
    update_additional_discount_on_tag(item_row = void 0) {
      const me = this;
    ...
    async update_paid_amount() {
      ...
      frappe.model.set_value(payments[0].doctype, payments[0].name, "amount", this.base_rounded_total).then(() => this.payment.update_totals_section(this.frm.doc));
    }
    ...
    get_item_from_frm({ name, item_code, batch_no, uom, rate, mrp, title, update_rules }) {
      ...
      if (name) {
        ...
        item_row = this.frm.doc.items.find(
          (i) => i.item_code === item_code && (!has_batch_no || has_batch_no && i.batch_no === batch_no) && i.uom === uom && i.qty > 0
        );
      }
      return item_row || {};
    }
    ...
    apply_pricing_rule_on_tag(doc) {
      ...
      var tag_name_list = void 0;
      var rules_name_list = void 0;
      frappe.call({
...
//# sourceMappingURL=pos_controller.bundle.LIB3OSFC.js.map
